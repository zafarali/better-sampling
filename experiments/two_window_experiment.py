...
from rvi_sampling.distributions.prior_distributions import MultiWindowDiscreteUniform
from rvi_sampling.plotting import determine_panel_size, visualize_proposal, multi_quiver_plot
from rvi_sampling.distributions.analytic_posterior import MultiWindowTwoStepRandomwWalkPosterior
...
if __name__=='__main__':
    ...
    if args.name != '':
        ...
        folder_name = 'two_window_results_{}'.format(time.strftime('%a-%d-%m-%Y__%H-%M-%s'))
    ...
    WINDOWS = [(-DISC_UNIFORM_WIDTH, -DISC_UNIFORM_WIDTH//2), (DISC_UNIFORM_WIDTH//2, DISC_UNIFORM_WIDTH)]
    def create_rw():
