...
#SBATCH --job-name=cycle_mc50k_MCSampler_VD
#SBATCH --ntasks=1
#SBATCH -o ./out_%x.txt
#SBATCH -e ./err_%x.txt
...
do
    ...
    -samseed $SLURM_ARRAY_TASK_ID -s $MC_SAMPLES -name rwseed0 -end_ov -endpoint $ENDPOINT --outfolder ${OUTFOLDER}_${ENDPOINT} --method MCSampler &
    python ./cycle_experiments/baseline_cycle_investigation.py --cycles $CYCLES --rw_seed 2 --rw_time $RW_TIME --rw_width $RW_WIDTH \
    -samseed $SLURM_ARRAY_TASK_ID -s $MC_SAMPLES -name rwseed2 -end_ov -endpoint $ENDPOINT --outfolder ${OUTFOLDER}_${ENDPOINT} --method MCSampler &
    python ./cycle_experiments/baseline_cycle_investigation.py --cycles $CYCLES --rw_seed 7 --rw_time $RW_TIME --rw_width $RW_WIDTH \
    -samseed $SLURM_ARRAY_TASK_ID -s $MC_SAMPLES --name rwseed7 -end_ov -endpoint $ENDPOINT --outfolder ${OUTFOLDER}_${ENDPOINT} --method MCSampler &
done
